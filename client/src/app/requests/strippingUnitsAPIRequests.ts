...
export const StrippingUnitsAPI = {
  ...
  fetchDataForDashboard: (fromDate: string, toDate: string) =>
    requests.get(
      `strippingdashboard/dashboard?fromDate=${fromDate}&toDate=${toDate}`
    ),

  fetchCsvData: (params?: { from: string; to: string }) =>
    requests.getBlob(`strippingdashboard/dashboard/csv`, params),
};
